        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 max-w-screen-xl mx-auto animate-fade-in bg-surface-900 min-h-screen">
            <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                    <div className="w-10 h-10 rounded-[2px] bg-brand-600 flex items-center justify-center shadow-glow shrink-0"><GraduationCap className="w-5 h-5 text-text-primary" /></div>
                    <div className="min-w-0"><h1 className="font-display font-bold text-xs sm:text-sm uppercase tracking-widest text-text-primary truncate">Admission Tracker</h1><p className="text-xs text-text-muted/70 mt-0.5">{admissions.length} applications</p></div>
                </div>
                <button onClick={() => { resetForm(); setShowForm(true); }} className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-brand-600 text-text-primary rounded-[2px] text-xs sm:text-sm font-medium hover:bg-brand-500 transition-none shadow-glow shrink-0"><Plus className="w-4 h-4" /><span className="hidden sm:inline"> Add University</span><span className="sm:hidden">Add</span></button>
            </div>
            {showForm && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                {admissions.map(a => (
                    <Card key={a.id} className="p-4 sm:p-5 bg-surface-800 border-surface-400/40 hover:border-brand-500/20 transition-all group">
                        <div className="flex items-start justify-between mb-2">
                            <div className="min-w-0 mr-2"><h3 className="text-sm font-bold text-text-primary truncate">{a.university}</h3><p className="text-xs text-text-muted/60 truncate">{a.program}</p></div>
                        ...
                        <div className="flex items-center justify-between mt-3 gap-2">
                            ...
                            {a.deadline && <span className="text-2xs text-text-muted/50 whitespace-nowrap">Due: {a.deadline}</span>}
                        </div>
