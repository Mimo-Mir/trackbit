        ...
        <div className="p-6 space-y-6 max-w-screen-xl mx-auto animate-fade-in bg-surface-900 min-h-screen">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-[2px] bg-brand-600 flex items-center justify-center shadow-glow"><Wallet className="w-5 h-5 text-text-primary" /></div>
                    <div><h1 className="font-display font-bold text-sm uppercase tracking-widest text-text-primary">Finance Tracker</h1><p className="text-xs text-text-muted/70 mt-0.5">{finances.length} transactions</p></div>
                </div>
                <button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-4 py-2 bg-brand-600 text-text-primary rounded-[2px] text-sm font-medium hover:bg-brand-500 transition-none shadow-glow"><Plus className="w-4 h-4" /> Add Transaction</button>
            </div>
            {/* Summary Cards */}
            <div className="grid grid-cols-3 gap-4">
                <Card className="p-4 bg-surface-800 border-surface-400/40"><p className="text-xs text-text-muted/60 mb-1">Income</p><p className="text-xl font-bold text-brand-400 flex items-center gap-2"><TrendingUp className="w-4 h-4" />${totalIncome.toFixed(2)}</p></Card>
                <Card className="p-4 bg-surface-800 border-surface-400/40"><p className="text-xs text-text-muted/60 mb-1">Expenses</p><p className="text-xl font-bold text-danger-400 flex items-center gap-2"><TrendingDown className="w-4 h-4" />${totalExpense.toFixed(2)}</p></Card>
                <Card className="p-4 bg-surface-800 border-surface-400/40"><p className="text-xs text-text-muted/60 mb-1">Balance</p><p className={cn("text-xl font-bold", totalIncome - totalExpense >= 0 ? "text-brand-400" : "text-danger-400")}>${(totalIncome - totalExpense).toFixed(2)}</p></Card>
            </div>
            {showForm && (
            ...
            <div className="flex gap-2">
                {(['all', 'Income', 'Expense'] as const).map(f => (
                    <button key={f} onClick={() => setFilter(f)} className={cn("px-4 py-1.5 rounded-[2px] text-xs font-medium transition-all capitalize", filter === f ? "bg-brand-600/20 text-brand-400 border border-brand-500/30" : "text-text-muted/60 hover:text-text-primary border-2 border-surface-400/50 hover:border-surface-400/50")}>{f}</button>
                ))}
            </div>
            <Card className="bg-surface-800 border-surface-400/40 overflow-hidden">
                <div className="divide-y divide-surface-400/20">
                    {filtered.map(f => (
                        <div key={f.id} className="flex items-center justify-between p-3 text-xs hover:bg-surface-700/50 transition-none group">
                            <div className="flex items-center gap-3">
                                <div className={cn("w-8 h-8 rounded-[2px] flex items-center justify-center", f.type === 'Income' ? "bg-brand-600/20" : "bg-danger-400/20")}>{f.type === 'Income' ? <TrendingUp className="w-4 h-4 text-brand-400" /> : <TrendingDown className="w-4 h-4 text-danger-400" />}</div>
                                <div><p className="text-text-primary font-medium">{f.description}</p><p className="text-2xs text-text-muted/50">{f.date} · {f.category}</p></div>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className={cn("font-bold", f.type === 'Income' ? "text-brand-400" : "text-danger-400")}>{f.type === 'Income' ? '+' : '-'}${f.amount.toFixed(2)}</span>
                                <button onClick={() => deleteFinance(f.id)} className="p-1.5 rounded-[2px] hover:bg-surface-700 text-text-muted/50 hover:text-danger-400 transition-none opacity-0 group-hover:opacity-100"><Trash2 className="w-3.5 h-3.5" /></button>
                            </div>
                        </div>
                    ))}
